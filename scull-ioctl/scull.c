...
struct scull_device{
    ...
    uint size;
    wait_queue_head_t block_queue;
    uint block_condition;
    struct mutex lock;
};
...
static ssize_t scull_read( struct file *file, char *buffer, size_t count, loff_t *pos)
{
    ...
    if(!nonblock)
    {
        printk(KERN_ERR "%s: wake_up! \n", __FUNCTION__);
        device->block_condition = 1;
        wake_up(&device->block_queue);
    }
    else
    {
        return -EAGAIN;
...
static ssize_t scull_write( struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    ...
    if( !nonblock)
    {
        device->block_condition = 0;
        ret = wait_event_interruptible(device->block_queue, device->block_condition);
        if( ret <0 )
        {
            printk (KERN_ERR "%s: wait_event: interrupted! \n",__FUNCTION__);
            return -EAGAIN;
        }
        else if (ret > 0)
        {
            printk(KERN_ERR "%s: wait_event: read come! \n",__FUNCTION__);
        }
        else
        {
            printk(KERN_ERR "%s: wait_event: time out! \n", __FUNCTION__);
        }
        return -EAGAIN;
    }
    else
...
static long scull_ioctl(struct file *file, uint cmd, ulong data )
{
    int ret=0, *p;
    int t;
    printk(KERN_ERR "%s :enter\n",__FUNCTION__);
    if( _IOC_TYPE(cmd) != IOCTL_JS_TYPE )
        return -ENOTTY;

    switch ( cmd ){
    case IOCTL_JS_WRITE_NORMAL:
        ret = 0x55;
           printk(KERN_ERR "ioctl_js_write_normal: input data = %x\n",data);
           return ret;
                
    case IOCTL_JS_WRITE_POINTER:
           p = (int *)data;
           ret = get_user(t, p);
           printk(KERN_ERR "ioctl_js_write_pointer: input data = %x\n",t);
           ret = put_user(0x56,p);
           return ret;
    default:
           return -ENOTTY;
...
static __init  int scull_init(void)
{
    ...
    for ( i=0; i< scull_num; i++)
    {
        ...
        init_waitqueue_head(&scull_devices[i].block_queue);
